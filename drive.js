...
// ============================================================
// TỰ ĐĂNG NHẬP LẠI SILENT KHI LOAD TRANG
// ============================================================
function driveInitSilent() {
  if (!localStorage.getItem(DRIVE_TOKEN_KEY)) return; // chưa từng đăng nhập
  if (!window.google) { setTimeout(driveInitSilent, 1500); return; }
  _ensureTokenClient(""); // silent - không hiện popup
  try { _driveTokenClient.requestAccessToken({ prompt: "" }); } catch {}
}

function _ensureTokenClient(promptMode) {
  ...
  _driveTokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: (resp) => {
      if (!resp.access_token) {
        if (promptMode !== "") showToast("Đăng nhập thất bại!", "error");
        return;
      }
...
// ============================================================
// ĐĂNG NHẬP THỦ CÔNG (hiện popup chọn tài khoản)
// ============================================================
function driveLogin() {
  if (!window.google) { showToast("Đang tải Google SDK, thử lại...", "warn"); return; }
  // Reset client để buộc hiện popup chọn tài khoản
  _driveTokenClient = null;
  _ensureTokenClient("select_account");
